...
throughout this project are placeholders (Learning Lab, PKI Lab Root CA, example.lab, etc.) — no personal information was ever entered into
...
it), and only caught it because git add failed with fatal: not a git repository. It's a good early reminder that in PKI work — same as in
...
bashopenssl genrsa -aes256 -out private/root-ca.key 4096
...
between the CA and anything it signs. For example, organizationName = match means the CA will refuse to sign a CSR claiming a different
...
bashopenssl req -x509 -new -key private/root-ca.key -sha256 -days 3650 \
...
bashopenssl x509 -in root-ca.crt -text -noout

Confirmed:


Issuer == Subject (C=US, ST=California, O=Learning Lab, CN=PKI Lab Root CA) — proof of self-signature.
...
Ran the initial directory-creation command in PowerShell, which doesn't
support Bash brace expansion ({certs,crl,...}}) — switched to Git Bash
and stayed there for consistency for the rest of the project.
The initial .gitignore was only the generic GitHub Python template — it
...
Status
 ...
 Phase 0 — CA directory structure & bookkeeping
 Phase 1 — Root CA (key + self-signed certificate)
